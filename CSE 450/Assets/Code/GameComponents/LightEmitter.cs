...
public class LightEmitter : MonoBehaviour
{
    ...
    public void EmitLight(Vector2 origin, Vector2 direction, int reflectionCount)
    {
        ...
        if (hit.collider != null)
        {
            ...
            else if (hit.collider.CompareTag("Mirror") && !hasReflected)
            {
                ...
                CheckPortalsActivation(hit.point, reflectedDirection);
                //CheckButtonsActivation(hit.point, reflectedDirection);
                Debug.Log("AAA" + reflectedDirection);
                //Debug.DrawRay(hit.point, normalizedNormal * lightRange, Color.blue, 1f);
                //Debug.Log("Normalized direction: " + normalizedDirection);
                //Debug.Log("Normalized normal: " + normalizedNormal);
                //Debug.Log("Hit point: " + hit.point);
    ...
    {
        List<ActButton> buttonsToDeactivate = new List<ActButton>();
        Debug.Log("BBB" + direction);
        foreach (ActButton button in activeButtons)
        {
            // 检查按钮是否为 null
            if (button == null || button.gameObject == null)
            {
                Debug.LogWarning("Button or its GameObject is null, skipping...");
                continue;
            }

            // 进行光线检测
            RaycastHit2D hit = Physics2D.Raycast(origin, direction, lightRange, actButtonLayer | groundAndPlayerLayer);

            // 如果没有检测到碰撞，或者碰撞对象不是当前按钮
            if (hit.collider == null)
            {

                Debug.Log("Raycast hit nothing.");
                buttonsToDeactivate.Add(button); // 光线未命中，标记为需要取消激活
            }
            else if (hit.collider.gameObject != button.gameObject)
            {
                Debug.Log("Hit something else: " + hit.collider.gameObject.name);
                Debug.Log("Button not activated: " + button.gameObject.name);
                buttonsToDeactivate.Add(button); // 光线未命中按钮，标记为需要取消激活
            }
        }
